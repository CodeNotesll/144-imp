...
int main() {
    try {
...

        {
            ...
            test.execute(SubmitSegment{"abcdefg", 0});
            test.execute(BytesAvailable("abcdefg"));
            test.execute(UnassembledBytes{0});
            test.execute(BytesAssembled(7)); 
        }

        {
            ...
            test.execute(SubmitSegment{"bcde", 10});
            test.execute(BytesAvailable(""));
            test.execute(BytesAssembled(0));
            test.execute(UnassembledBytes(8)); 

            test.execute(SubmitSegment{"ghbcde", 8});
            test.execute(BytesAvailable(""));
            test.execute(BytesAssembled(0));
            test.execute(UnassembledBytes(10));
            
            test.execute(SubmitSegment{"a", 0});
            test.execute(BytesAvailable("abcde"));
            test.execute(BytesAssembled(5));
            test.execute(UnassembledBytes(6));
        ...
        }
        {
            ...
            test.execute(SubmitSegment{"makes every", 13});
            test.execute(BytesAvailable(""));
            test.execute(BytesAssembled(0));
            test.execute(UnassembledBytes(2));

            test.execute(SubmitSegment{"makes", 0});
            test.execute(BytesAvailable("makes"));
            test.execute(BytesAssembled(5));
            test.execute(UnassembledBytes(2)); 

            test.execute(SubmitSegment{"lmakes every", 12});
            test.execute(BytesAvailable(""));
            test.execute(BytesAssembled(5));
            test.execute(UnassembledBytes(8));            
        }
